...
npie_status_t npie_detect_accelerators(npie_context_t ctx,
    ...
    *count = total;
    ...
    if (ctx->accelerators_detected == false) {
        ctx->accelerator_count = total;
        for (uint32_t i = 0; i < total && i < 16; i++) {
            ctx->available_accelerators[i] = accelerators[i];
        }
        ctx->accelerators_detected = true;
    }
...
bool npie_is_accelerator_available(npie_context_t ctx, npie_accelerator_t accelerator) {
    ...
    /* Detect accelerators if not already done */
    if (!ctx->accelerators_detected) {
        npie_accelerator_t accel_list[16];
        uint32_t count;
        npie_detect_accelerators(ctx, accel_list, 16, &count);
    }
    
    /* Check if requested accelerator is in the list */
    for (uint32_t i = 0; i < ctx->accelerator_count; i++) {
        if (ctx->available_accelerators[i] == accelerator) {
            return true;
        }
    }
